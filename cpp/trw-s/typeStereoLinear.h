...
#ifndef __TypeStereoLinear_H__
#define __TypeStereoLinear_H__
...
public:
		...
		//////////////////////////////////////////////////////////////////////////////////
		friend class MRFEnergy<TypeStereoLinear>;
...
inline TypeStereoLinear::GlobalSize::GlobalSize(int K)
{
		m_K = K;
}
inline TypeStereoLinear::LocalSize::LocalSize(int K)
{
		m_K = K;
}
...
inline TypeStereoLinear::NodeData::NodeData(REAL* data)
{
		m_data = data;
}

inline TypeStereoLinear::EdgeData::EdgeData(REAL lambda, REAL alpha, REAL* stacked_data, int* stacked_indices)
{
		m_alpha  = alpha;
		m_lambda = lambda;
...
inline int TypeStereoLinear::Vector::GetSizeInBytes(GlobalSize Kglobal, LocalSize K)
{
		if (K.m_K < 1)
		{
				return -1;
...
inline void TypeStereoLinear::Vector::Initialize(GlobalSize Kglobal, LocalSize K, NodeData data)
{
		memcpy(m_data, data.m_data, K.m_K*sizeof(REAL));
}

inline void TypeStereoLinear::Vector::Add(GlobalSize Kglobal, LocalSize K, NodeData data)
{
...
inline void TypeStereoLinear::Vector::SetZero(GlobalSize Kglobal, LocalSize K)
{
		memset(m_data, 0, K.m_K*sizeof(REAL));
}

inline void TypeStereoLinear::Vector::Copy(GlobalSize Kglobal, LocalSize K, Vector* V)
{
		memcpy(m_data, V->m_data, K.m_K*sizeof(REAL));
}

inline void TypeStereoLinear::Vector::Add(GlobalSize Kglobal, LocalSize K, Vector* V)
{
		for (int k=0; k<K.m_K; k++)
		{
				m_data[k] += V->m_data[k];
...
inline TypeStereoLinear::REAL TypeStereoLinear::Vector::GetValue(GlobalSize Kglobal, LocalSize K, Label k)
{
		assert(k>=0 && k<K.m_K);
		return m_data[k];
}

inline TypeStereoLinear::REAL TypeStereoLinear::Vector::ComputeMin(GlobalSize Kglobal, LocalSize K, Label& kMin)
{
		REAL vMin = m_data[0];
		kMin = 0;
...
inline TypeStereoLinear::REAL TypeStereoLinear::Vector::ComputeAndSubtractMin(GlobalSize Kglobal, LocalSize K)
{
		REAL vMin = m_data[0];
		for (int k=1; k<K.m_K; k++)
		{
...
inline int TypeStereoLinear::Edge::GetSizeInBytes(GlobalSize Kglobal, LocalSize Ki, LocalSize Kj, EdgeData data)
{
...
inline int TypeStereoLinear::Edge::GetBufSizeInBytes(int vectorMaxSizeInBytes)
{
...
inline void TypeStereoLinear::Edge::Initialize(GlobalSize Kglobal, LocalSize Ki, LocalSize Kj, EdgeData data, Vector* Di, Vector* Dj)
{
...
inline TypeStereoLinear::Vector* TypeStereoLinear::Edge::GetMessagePtr()
{
		return m_message;
}

inline void TypeStereoLinear::Edge::Swap(GlobalSize Kglobal, LocalSize Ki, LocalSize Kj)
{
		((Edge*)this)->m_dir = 1 - ((Edge*)this)->m_dir;
}


inline TypeStereoLinear::REAL TypeStereoLinear::Edge::Smooth(REAL val)
{
				return ( m_alpha* min( std::abs(val) , m_lambda) );
}

inline TypeStereoLinear::REAL TypeStereoLinear::Edge::UpdateMessage(GlobalSize Kglobal,
				LocalSize Ksource,
				LocalSize Kdest,
				Vector* source,
				REAL gamma,
				...
				void* _buf)
{
	...
	// Variables used inside the algorithm
	REAL* H = (REAL*) _buf;
	...
	// Init
	j = 0;
	v[0] = q_order[0]; // Lower envelope id
	z[0] = -std::numeric_limits<REAL>::infinity(); // Boundary of lower i is given by z[i],z[i+1].
	z[1] = std::numeric_limits<REAL>::infinity();


	// Read position of "roots".
	...
	} else
	{
		...
		for (k = 1; k <  Kglobal.m_K; k++)
		{
			...
			// Upper bound on how many steps it has to go back.
			for (l = k; l >= 0; l--)
			{
				// Compare height
				hj = H[ v[j] ];
				...
				// qj < qk always.
				dist = m_alpha*std::abs(qk-qj);

				// j segment is above this line
				if ( (dist+hk) < hj)
				{
					// Special case we replace leftmost segment
					if (j == 0)
					{
						v[0] = q_order[k];
				...
				// This segment is above the current lower envelope
				// This segment will never be usefull
				else if ( (dist+hj) <= hk )
				{
						break;
				}

				// This segment will create a new part of the lower envelope
				else
				{
					s = ((hk - hj) + m_alpha * (qk + qj))/(2*m_alpha);
		...
		for (k = 0; k <  Kglobal.m_K; k++)
		{
			...
			m_message->m_data[ qprim_order[k] ] =
					min( vTrunc, m_alpha*std::abs(qprimk - qj) + hj);

			vMin = min( vMin, m_message->m_data[ qprim_order[k] ] );
		}
	}
...
// If dir==0, then sets dest[kj] += V(ksource,kj).
// If dir==1, then sets dest[ki] += V(ki,ksource).
inline void TypeStereoLinear::Edge::AddColumn(GlobalSize Kglobal, LocalSize Ksource, LocalSize Kdest, Label ksource, Vector* dest, int dir)
{
